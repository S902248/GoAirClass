...
const Hero = ({ setSearchParams }) => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (!event.target.closest('.relative') && !event.target.closest('.calendar-container')) {
    ...
    const generateCalendarDays = () => {
        ...
        const adjustedFirstDay = firstDay === 0 ? 6 : firstDay - 1; // Adjust to Mon start
                    ...
                    <div className="flex flex-col lg:flex-row w-full bg-white/95 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-2xl border border-white/20">
                        ...
                        <div className="flex lg:w-1/4 bg-transparent border-t lg:border-t-0 border-slate-100 relative">
                            <div
                                ...
                                className="flex items-center px-6 py-5 cursor-pointer hover:bg-slate-50/80 transition-colors group flex-1"
                            >
                                <div className="bg-slate-100 p-2.5 rounded-xl mr-4 group-hover:bg-[#f26a36]/10 transition-colors">
                            ...
                            {showCalendar && (
                                <div className="calendar-container absolute top-[calc(100%+15px)] left-0 lg:left-[-50px] w-[320px] bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] border border-slate-100 z-[110] p-5 animate-in fade-in slide-in-from-top-4 duration-300">
                                                ...
                                                {day ? (
                                                    <button
                                                        ...
                                                        className={`w-8 h-8 flex items-center justify-center rounded-full text-[13px] font-bold transition-all
                                                            ${isSelected(day) ? 'bg-[#f26a36] text-white shadow-lg' :
                                                                isWeekend(day) ? 'text-[#D84E55] hover:bg-red-50' :
                                                                    'text-slate-700 hover:bg-slate-50'}
                                                            ${isToday(day) && !isSelected(day) ? 'border border-[#f26a36]' : ''}
                                                        `}
                                                    >
                                                        {day.getDate()}
                                                    </button>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between">
                                        <button
                                            ...
                                            className="text-[11px] font-bold text-slate-400 hover:text-[#f26a36] transition-colors"
                                        >
                                            Today
                                        </button>
                                        <button
...
// Add styles for animation and click outside handling could be done via a hook or effect
export default Hero;
