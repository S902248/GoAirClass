...
const CancelReportView = () => {
  ...
  const cancelledBookings = useMemo(() => {
    return bookings.filter(b => b.status === 'Cancelled' || b.paymentStatus === 'Cancelled');
  }, [bookings]);
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Status',
      render: () => (
        <span className="px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-400">
          CANCELLED
        </span>
      ),
    },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white uppercase tracking-widest">Cancelled Reports</h1>
          </div>
          <p className="text-slate-500 mt-2 font-medium">Monitoring track of all refunded and aborted travel transactions.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button className="bg-primary-600 text-white px-5 py-2 rounded-xl text-sm font-bold shadow-lg shadow-primary-500/20 hover:bg-primary-700 transition-all flex items-center gap-2">
            <CardIcon size={16} /> Export PDF
          </button>
        </div>
      </div>

      <div className="card shadow-2xl shadow-rose-100/50 dark:shadow-none border-none overflow-hidden">
        <DataTable
          columns={columns}
          data={cancelledBookings}
          isLoading={loading}
          onRowClick={(booking) => console.log('Cancelled booking detail:', booking)}
        />
      </div>
    </div>
  );
...
export default CancelReportView;
